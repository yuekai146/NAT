...
from utils import nat_get_batch
import optimizer
import model
import dataset
...
class Trainer(object):
    ...
    def optimize(self, loss):
        ...
        ngpus = float(os.environ['NGPUS'])
        accum = float(config.accumulate_gradients)
        loss = loss / (ngpus * accum)
...
class Enc_Dec_Trainer(Trainer):
    ...
    def soft_copy(self, src_emb, src_mask, tgt_mask, tau=config.tau):
        # src_emb: torch.FloatTensor (bzs, src_len, emb_dim)
        # src_mask: torch.ByteTensor (bsz, src_len)
        # tgt_mask: torch.ByteTensor (bsz, tgt_len)
        # tau: float number

        # Return: FloatTensor (bsz, tgt_len, emb_dim)
        bsz, src_len, emb_dim = src_emb.size()
        tgt_lengths = tgt_mask.sum(-1).long()
        tgt_len = tgt_lengths.max()
        src_pos = torch.stack(
        ...
        mask = tgt_mask.unsqueeze(-1) & src_mask.unsqueeze(-2)
        soft_weights = soft_weights.masked_fill(mask == 0, -float('inf'))
        soft_weights = F.softmax(soft_weights, dim=-1)

        return torch.matmul(soft_weights, src_emb)


    def hsp(self, tgt_emb, tgt_mask, dec_input):
        ...
        def hsp_for_one_sent(cos_mat, slen, sent_idx):
            ...
            res = []
            for i in range(slen):
                best_pos = np.argmax(cos_mat)
                best_row = best_pos // (slen - i)
                best_col = best_pos % (slen - i)
                res.append((best_row, best_col))

                if slen - i - 1 > 0:
                    cos_mat = np.delete(cos_mat, best_row, 0)
                    cos_mat = np.delete(cos_mat, best_col, 1)
    ...
    def nat_step(self, raw_batch):
        ...
        tensor = self.net(batch['src'], batch['src_mask'], batch['target'], batch['target_mask'], batch['tgt_mask'])
        # loss
    ...
    def valid_step(self):
        ...
        with torch.no_grad():
            ...
            for i_batch, raw_batch in enumerate(data_iter):
                ...
                # Network forward step
                logits = self.net(batch['src'], batch['src_mask'], batch['target'], batch['target_mask'], batch['tgt_mask'])

                # loss
                loss, nll_loss = self.criterion(logits, batch['target'], batch['target_mask'])
