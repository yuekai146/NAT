...
from utils import remove_bpe, remove_special_tok, subsequent_mask, nat_get_batch
...
def nat_translate(args, net, src, tgt, src_vocab, tgt_vocab):
    batch = nat_get_batch(src, tgt, src_vocab, tgt_vocab) 
    with torch.no_grad():
        net.eval()
        logits = net(
                batch['src'], batch['src_mask'], batch['target'],
                batch['target_mask'], batch['tgt_mask']
                )
        generated = logits.max(-1)[1]
        gen_len = batch['target_mask'].sum(-1).long()
...
def translate(args, net, src_vocab, tgt_vocab):
    ...
    for raw_batch in infer_dataiter:
        ...
        if args.greedy:
            ...
            generated, gen_len = nat_translate(args, net, src, raw_batch.tgt, src_vocab, tgt_vocab)
        else:
